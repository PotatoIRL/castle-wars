 ...
 * SimpleLogger. Must be one of ("trace", "debug", "info", "warn", or "error"). If not specified,
 * defaults to "info".</li>
 *
 ...
 * "warn", or "error". When a SimpleLogger named "a.b.c" is initialized, its level is assigned from
 * this property. If unspecified, the level of nearest parent logger will be used, and if none is
 * set, then the value specified by <code>org.slf4j.simpleLogger.defaultLogLevel</code> will be
 * used.</li>
 *
 * <li><code>org.slf4j.simpleLogger.showDateTime</code> - Set to <code>true</code> if you want the
 * current date and time to be included in output messages. Default is <code>true</code></li>
 *
 ...
 * the last component of the name to be included in output messages. Defaults to <code>false</code>.
 * </li>
 *
...
public class SimpleLogger extends MarkerIgnoringBase {
    ...
    static void init() {
        ...
        if (DATE_TIME_FORMAT_STR != null) {
            try {
                ...
                Util.report(
                        "Bad date format in " + CONFIGURATION_FILE + "; will output relative time",
                        e);
            }
        }
    }
    ...
    private static void loadProperties() {
        ...
        InputStream in =
                (InputStream) AccessController.doPrivileged(new PrivilegedAction<Object>() {
                    @Override
                    public Object run() {
    ...
    private void log(int level, String message, Throwable t) {
        ...
        StringBuffer msgBuf = new StringBuffer(32);

        // Append date-time if so configured
        if (SHOW_DATE_TIME) {
            if (DATE_FORMATTER != null) {
                msgBuf.append(getFormattedDate());
                msgBuf.append(' ');
            } else {
                msgBuf.append(System.currentTimeMillis() - START_TIME);
                msgBuf.append(' ');
            }
        }
        ...
        if (SHOW_THREAD_NAME) {
            msgBuf.append('[');
            msgBuf.append(Thread.currentThread().getName());
            msgBuf.append("] ");
        }
        ...
        if (SHOW_SHORT_LOG_NAME) {
            ...
            msgBuf.append(String.valueOf(name)).append(" - ");
        }
        ...
        if (t == null) {
            Gdx.app.log(levelTag, msgBuf.toString());
        } else {
            Gdx.app.log(levelTag, msgBuf.toString(), t);
        }
    ...
    /**
     * Perform single parameter substitution before logging the message of level INFO according to
     * the format outlined above.
     */
    @Override
    ...
    /**
     * Perform double parameter substitution before logging the message of level INFO according to
     * the format outlined above.
     */
    @Override
    ...
    /**
     * Perform single parameter substitution before logging the message of level WARN according to
     * the format outlined above.
     */
    @Override
    ...
    /**
     * Perform double parameter substitution before logging the message of level WARN according to
     * the format outlined above.
     */
    @Override
