...
public class LegacyGameRenderingSystem {
    ...
    private void renderEntity(float dt, Object e) {
        RenderableComponent avatar = null;
        ...
        if (renderable == null) {
            ...
            batch.draw(renderable,
                    (aabb.x() + aabb.w() / 2) - (renderable.getRegionWidth() / 2),
                    (aabb.y() + aabb.h() / 2) - (renderable.getRegionHeight() / 2),
                    (renderable.getRegionWidth() / 2),
                    (renderable.getRegionHeight() / 2),
                    renderable.getRegionWidth(),
                    renderable.getRegionHeight(),
                    (avatar.flipped ? -1 : 1), 1, 0);
        }
    }
