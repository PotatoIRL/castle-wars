...
import com.phfl.game.asset.loader.PyxelFileLoader;
...
public class GameAssets {
  ...
  public static <T> T get(String filePath, Class<T> fileType) {
    contextualize();
    if (!manager.isLoaded(filePath)) {
      LOGGER.warn("[get] triggered [load] for filePath=[{}], fileType=[{}]", filePath, fileType);
  ...
  private static AssetManager defaultAssetManager() {
    ...
    defaultManager.setLoader(PyxelFile.class, new PyxelFileLoader(resolver));
